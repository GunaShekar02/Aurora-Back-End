...
const { canViewAcc } = require('../../../utils/roles');

const accOrders = async (_, args, context) => {
  const { isValid, id, userLoader, db } = context;

  if (isValid && (await canViewAcc(id, userLoader))) {
